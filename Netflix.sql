...
SELECT
	...
	COUNT(show_id) as Total_country
FROM netflix
GROUP BY country
ORDER BY 2 DESC
LIMIT 5
...
SELECT
	...
	COUNT(show_id) AS number_od_content
FROM netflix
GROUP BY genre
...
--13. Find how many movies actor 'Kevin kline' appeared in last 10 years!

SELECT
	* 
FROM netflix
WHERE casts ILIKE '%Kevin kline%'
	AND
...
AS(
	...
	*,
		...
		WHEN description ILIKE '%kill%' OR 
			description ILIKE '%violence%' THEN 'BasContent'
			ELSE 'Good Conten'
		END category
	FROM netflix
)
